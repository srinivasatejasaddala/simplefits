...
const Kid = () => {
    ...
    const [kid_products, setKid_Products] = useState([])
    ...
    useEffect(() => {
          ...
          if (kid_products.length===0) {
             axios.get(`${process.env.REACT_APP_API_URL}/kid`).then((response) =>{ setKid_Products(response.data)
    ...
    return (
        <>  
                   <Navbar />
    {kid_products.length === 0 ? <Loadding /> :
        <div>
            <div className='w-fit m-auto mt-20 mb-32'>
                <img className='w-[85%] md:w-[75%] lg:w-[75%] xl:w-[75%]  m-auto ' src={kidbanner}  />
            </div>
            ...
            <div>
                <div className='grid sm:w-[70%] grid-rows-2 grid-cols-2 w-[99%]  md:grid-cols-3 lg:grid-rows-1 lg:grid-cols-4 gap-x-3 gap-y-20 md:w-fit lg:w-fit xl:w-fit m-auto items-center mt-10 mb-32' >
                        {kid_products.length > 0 ? kid_products.map(x =>  <Link to='/single-item' state={{from:x,signel:"not_cart"}}>    
